...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState({ users: 0, tickets: 0, pedidosPendentes: 0, parceriasPendentes: 0, feedbackPendentes: 0, faturasPendentes: 0 });
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const [{ data: cData }, { data: tData }, { data: cpData }, { count: uCount }, { count: tOpenCount }, { count: pedidosCount }, { count: parceriasCount }, { count: feedbackCount }, { data: faturasData, count: faturasCount }] = await Promise.all([
        ...
        supabase.from('prod_feedback').select('*', { count: 'exact', head: true }).eq('estado', 'pendente'),
      ...
      setStats({ users: uCount || 0, tickets: tOpenCount || 0, pedidosPendentes: pedidosCount || 0, parceriasPendentes: parceriasCount || 0, feedbackPendentes: feedbackCount || 0, faturasPendentes: faturasCount || 0 });
    } catch (err) {
  ...
  return (
    <div style={s.page}>
      ...
      <aside style={s.sidebar}>
        ...
        >
          <span>💬 Feedback</span>
          {stats.feedbackPendentes > 0 && (
            <span style={{ background: '#f59e0b', color: '#000', fontSize: 10, fontWeight: 800, padding: '2px 7px', borderRadius: 20 }}>
              {stats.feedbackPendentes}
            </span>
          )}
        </Link>

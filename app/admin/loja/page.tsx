                  ...
                  return (
                    <tr key={p.id}>
                      <td style={s.td}>
                        ...
                        <div style={{ fontSize: 12, color: '#8a96aa' }}>/{p.slug}</div>
                      </td>
                      ...
                      <td style={s.td}>
                        {p.preco_promo_cents != null ? (
                          <span><span style={{ color: '#34d399', fontWeight: 700 }}>{eur(p.preco_promo_cents)}</span>{' '}
                            <span style={{ textDecoration: 'line-through', color: '#8a96aa', fontSize: 12 }}>{eur(p.preco_cents)}</span></span>
                        ) : eur(p.preco_cents)}
                      </td>
                      <td style={s.td}>
                        <span style={{ color: st === 0 ? '#f87171' : st < 5 ? '#fbbf24' : '#cbd5e1', fontWeight: 600 }}>{st}</span>
                        {p.prod_loja_variantes.length > 0 && <span style={{ fontSize: 11, color: '#8a96aa' }}> ({p.prod_loja_variantes.length} var.)</span>}
                      </td>
                      <td style={s.td}>
                        <div style={{ fontSize: 12, color: prazo.tipo === 'stock' ? '#34d399' : '#fbbf24', fontWeight: 700 }}>{prazo.label}</div>
                        <div style={{ fontSize: 11, color: '#8a96aa' }}>{prazo.dias}</div>
                      </td>

...
export default function EditorControls({ produto, valores, onUpdate }: EditorControlsProps) {
  ...
  // Inicializa valores quando muda o produto (id)
  useEffect(() => {
    const schema = produto?.ui_schema ?? [];
    const iniciais: ValoresProduto = { ...(produto.parametros_default ?? {}) };

    // aplica defaults por campo (sem meter arrays/objects no state)
    for (const c of schema) {
      ...
      if (iniciais[c.name] === undefined) {
        if (c.value !== undefined && (typeof c.value === 'string' || typeof c.value === 'number' || typeof c.value === 'boolean')) {
          ...
          // valor vazio por omissão
          iniciais[c.name] = c.type === 'slider' ? 0 : '';
        }
      }
    }
  ...
  // Mantém sincronizado se o pai atualizar valores externamente
  useEffect(() => {
    if (!valores) return;
    // só atualiza se for diferente (evita loops)
  ...
  const fields = useMemo(() => {
    return (produto.ui_schema ?? []).filter((c) => c && c.name);
  }, [produto.ui_schema]);
  ...
  // ✅ fallback: se não houver secções no schema, mostra tudo em "Geral"
      ...
      {seccoesParaRender.map((sec) => {
        ...
        return (
          <div key={sec} style={{ marginBottom: 18 }}>
            <div style={{ fontWeight: 900, margin: '8px 0 10px', color: 'white' }}>
              {sec}
            </div>
                    ...
                    <div key={c.name}>
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          ...
                          />
                          {unit ? (
                            <span style={{ fontSize: 12, opacity: 0.75, whiteSpace: 'nowrap', color: '#cbd5e1' }}>
                              {unit}
                            </span>
                          ) : null}
                        </div>
